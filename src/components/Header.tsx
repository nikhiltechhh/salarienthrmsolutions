...
const Header = () => {
  ...
  const navItems: NavItem[] = [
    ...
    { name: "Pricing", hasDropdown: false, href: "/payment" }, 
    { 
      name: "Partners", 
      hasDropdown: true,
  ...
  return (
    ...
    >
      <div className="container flex h-16 items-center justify-between px-4 md:px-8">
            ...
            <div key={item.name} className="relative">
              {item.hasDropdown ? (
                <button
                  ...
                  className="flex items-center gap-1 text-sm font-medium text-foreground transition-colors hover:text-primary py-2"
                >
                  {item.name}
                  <ChevronDown 
                    className={cn(
                    ...
                    <div className="p-2">
                      {item.dropdownItems?.map((dropdownItem, index) => (
                        <a
                          key={index}
                          href="#"
                          className="block rounded-md px-4 py-2.5 text-sm text-foreground hover:bg-accent hover:text-accent-foreground"
                        >
                          {dropdownItem}
                        </a>
                      ))}
        ...
        >
          <span className={cn("h-0.5 w-6 bg-foreground", mobileMenuOpen && "translate-y-[7px] rotate-45")} />
          <span className={cn("h-0.5 w-4 bg-foreground", mobileMenuOpen && "opacity-0")} />
          <span className={cn("h-0.5 w-6 bg-foreground", mobileMenuOpen && "-translate-y-[7px] -rotate-45")} />
        </button>
        ...
        <div
          className={cn(
            "fixed inset-x-0 top-16 z-40 max-h-[calc(100vh-4rem)] overflow-y-auto border-b border-border bg-background shadow-lg transition-all duration-300 lg:hidden",
              ...
              <div key={item.name} className="border-b border-border/50 pb-2 last:border-0">
                {item.hasDropdown ? (
                  <>
                    <button
                      onClick={() => toggleDropdown(item.name)}
                      className="flex w-full items-center justify-between py-2 text-left text-sm font-medium hover:text-primary"
                    >
                      {item.name}
                      <ChevronDown 
                        className={cn(
